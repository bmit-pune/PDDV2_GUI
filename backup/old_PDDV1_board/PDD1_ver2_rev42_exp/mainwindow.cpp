...
for ( i = min; i < max  ; i++ ) {
    ...
    if(mode==SCREENING && (i%this->lut.skip_value[Present_Zoom_Level])==0) {  /*temp*/
        if(((dialog.Set_Threshold/4)+173) >(int)array[i])
...
if(mode==SCREENING ) {
if((((dialog.Set_Threshold/4)+173)+4)>250)
    ui->customPlot->yAxis->setRangeUpper(190);
else
ui->customPlot->yAxis->setRangeUpper(((dialog.Set_Threshold/4)+173)+4);
ui->customPlot->yAxis->setRangeLower(165);
...
qDebug("LOC_MON %d",rand()%10);
    ...
    if(loc.iteration< (dialog.Set_Time*12))
    {
        ...
        for(int i=0;(i<NO_OF_RAW_VALS) ;i++)
        {
            ...
            if(arr_float_mV[i]>= 0 && arr_float_mV[i]< 12) //for owts
                owts.owts_array[0][loc.location]++;
            else if(arr_float_mV[i]>12 && arr_float_mV[i]<24)
                owts.owts_array[1][loc.location]++;
            else if(arr_float_mV[i]>24 && arr_float_mV[i]<36)
                owts.owts_array[2][loc.location]++;
            else if(arr_float_mV[i]>36 && arr_float_mV[i]<48)
                owts.owts_array[3][loc.location]++;
            else if(arr_float_mV[i]>48 /*&& arr_float_mV[i]<60*/)
...
#define LABELHEIGHT 25
    ...
    QPixmap unknown("/home/linaro/oct-16/new/unknown.png");
        ...
        QFont my_font( "Helvetica [Cronyx]", 6, QFont::Cursive);
