...
/*--------------------------------------------------------------*/
    ...
    for(int i=0;i<NO_OF_PLOTTING_VALS;i++)
         {
            if(mode==SCREENING || mode==LOC_MON)
                array[i]=131 + ( std::rand() % ( 191 - 131 + 1 ) );
            if(mode==SPECTRUM)
    ...
    if(mode==LOC_MON)
    {
        for(int i=0;i<NO_OF_FFT_VALS;i++){
            ...
            /*if(i%8==0)
                qDebug("mV=%f=",(float)arr_float_mV[i]);*/


        }
        qDebug("Finished filling\n");
    }
...
inline float MainWindow::sample_to_millivolt(unsigned int sample) {
    ...
    //qDebug("*rec sample:%d*\n",sample);
    //qDebug("<->%f<->",(ADC_REF_VOLTAGE/pow(2,ADC_BIT))*sample);

    signed int temp=sample-172;
    if(temp<0)
        temp=temp*-1;
