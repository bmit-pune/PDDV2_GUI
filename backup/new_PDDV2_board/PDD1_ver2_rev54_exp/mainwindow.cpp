...
void MainWindow::realtimeDataSlot()
{
    ...
    gps_counter++;
    if (gps_counter >= 23) { //enter every 23rd iteration
...
void MainWindow::update_display_array(int clicked_pixel_point, SCROLL scroll)
{
    ...
    case IN: /*qDebug("in**");*/
        ...
        if (min < boundry_lower) { /*qDebug("inside IN if");*/
            min = boundry_lower;
            max = min + this->lut.L[Present_Zoom_Level + 1];
            Present_Zoom_Level++;
            break;
        }

        max = clicked_pixel_point + (int)(this->lut.L[Present_Zoom_Level + 1] / 2);
        if (max > boundry_upper) { /*qDebug("inside max IN if");*/
            max = boundry_upper;
            min = max - this->lut.L[Present_Zoom_Level + 1];
        }
        Present_Zoom_Level++;
        break;
    ...
    case OUT: /*qDebug("out**");*/
        ...
        if (min < boundry_lower) { /*qDebug("inside min oUT if");*/
            min = boundry_lower;
            max = (int)(this->lut.L[Present_Zoom_Level - 1]);
            Present_Zoom_Level--; /*qDebug("MIn:%d",min);qDebug("Max:%d",max);*/
            break;
        }

        max = clicked_pixel_point + (this->lut.L[Present_Zoom_Level]);
        if (max > boundry_upper) { /*qDebug("inside max oUT if");*/
            max = boundry_upper;
            min = max - (int)(this->lut.L[Present_Zoom_Level - 1]);
        }
        Present_Zoom_Level--;
        break;
    ...
    case CLICKZOOM:
        min = clicked_pixel_point - 256;
        max = clicked_pixel_point + 256;
