...
void ThresholdTime::keyPressEvent(QKeyEvent *e)
{
    ...
    if(e->key()==Qt::Key_I && (e->isAutoRepeat() == false))
    {
        if(pos==0)
        {
    ...
    else if(e->key()==Qt::Key_D && (e->isAutoRepeat() == false))
    {
        if(pos==0)
        {
            row=ui1->listView->currentIndex().row();
...
else if(e->key()==Qt::Key_M && (e->isAutoRepeat() == false))//enter
{
