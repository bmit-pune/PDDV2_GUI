...
void MainWindow::start_LOC_MON_slot(){
    ...
    if(loc.iteration< (dialog.Set_Time*12))
    {
        ...
        for(int i=0;(i<NO_OF_RAW_VALS) ;i++)
        {
            ...
            else if(arr_float_mV[i]>3 && arr_float_mV[i]<5)
                owts.owts_array[1][loc.location]++;
            else if(arr_float_mV[i]>5 && arr_float_mV[i]<7)
                owts.owts_array[2][loc.location]++;
            else if(arr_float_mV[i]>7 && arr_float_mV[i]<9)
                owts.owts_array[3][loc.location]++;
            else if(arr_float_mV[i]>9 /*&& arr_float_mV[i]<60*/)
...
QPixmap tick("/opt/new/yellow_tick.jpg");
    ...
    for(int i=0;i<5;i++){
        switch(i){
        case 0:
        owts.label[*(recv_index + i)][location]->setPixmap(cross);break;
        case 1:
        owts.label[*(recv_index + i)][location]->setPixmap(circle);break;
        case 2:
        owts.label[*(recv_index + i)][location]->setPixmap(tick);break;
        case 3:
        owts.label[*(recv_index + i)][location]->setPixmap(square);break;
        case 4:
        owts.label[*(recv_index + i)][location]->setPixmap(triangle);break;
        }
