...
void MainWindow::update_data()
{
    ...
    uint32_t mem_address;
        ...
        *control_reg_addr = 0x00000001 ; //write control register bit to start pipeline ADC ON
        gettimeofday(&t0,0);
        QThread::msleep(50);
        ...
        *control_reg_addr = 0x00000002; //To indicate uP started reading ADC OFF
...
/*--------------------------------------------------------------*/
         ...
         if(mode==SCREENING || mode==LOC_MON){
             for(int i=0;i<NO_OF_RAW_VALS;i++)
             array[i]= std::rand() % ( 32970 + 1 - 32740 ) + 32740 ;
         }
...
inline float MainWindow::sample_to_millivolt(unsigned int sample) {
    ...
    signed int temp=sample-32708;//32747
    if(temp<0)
        temp=temp*-1;   
