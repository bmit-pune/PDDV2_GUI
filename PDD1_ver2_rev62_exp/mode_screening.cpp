...
void MainWindow::update_display_array(int clicked_pixel_point, SCROLL scroll)
{
    ...
    for (i = graph_min; i < graph_max; i++) {
        ...
        if (i > ((2 * KILO_SAMPLES)-3) && mode == SPECTRUM)
            continue;
        ...
        if (mode == SPECTRUM ) {
            ...
            y0.push_back((float)arr_float_sorted[i]);
         //   qDebug("pushed back : %d->%f\n",i,y0[i]);
    ...
    if(GRAPH_MIN_MAX_CHANGED==1)
        ...
        if(/*GRAPH_MIN_MAX_CHANGED==1 || (mode==SPECTRUM && NOISE_FLAG==1)*/1){
       ...
       /* for(i=0;i<512;i++){
            qDebug("Key1[%d]=%d",i,key1[i]);
            qDebug("y0[%d]=%f",i,y0[i]);
        }*/
...
int* MainWindow::index_sort()
{

    std::pair<float, int> pair[NO_OF_FFT_VALS/2];
   static int array_sort_index[NO_OF_FFT_VALS/2];

    for (int i = 0; i < NO_OF_FFT_VALS/2; i++) {
        pair[i].first = arr_float[i];
        pair[i].second = i;
    }
    ...
    std::sort(pair, pair + (NO_OF_FFT_VALS/2) -1);
    ...
    memset(arr_float_sorted,0,sizeof(arr_float_sorted));
    for (int i = 0; i < NO_OF_FFT_VALS/2; i++){
        array_sort_index[i] = pair[i].second;
        if(i>(((2*KILO_SAMPLES) - 60)) && i<=2*KILO_SAMPLES){
            if(arr_float[array_sort_index[i]] < 20000000){
        arr_float_sorted[array_sort_index[i]]=arr_float[array_sort_index[i]];
           // qDebug("arr_float_sorted[%d]=%f\n",array_sort_index[i],arr_float_sorted[array_sort_index[i]]);
            }
