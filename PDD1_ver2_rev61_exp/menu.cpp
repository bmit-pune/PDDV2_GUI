    ...
    ui2(new Ui::menu)
{
    ...
    List4 << "Exit"<<"Press SLEEP for power saver"<<"longpress SHUTDOWN to shutdown"<<"SCR default->FFT mode SCR once->TimeDomain"<<"LOC monitor before OWTS"<<"Save data before mode switching"<<"and when battery low";
    // Populate our model
    ...
    pos=0;
    clean_ok_flag=0;
    ui2->label->setText("Press Menu to Select \nRemaning Memory(%):"+QString::fromUtf8(exec("df -kh | grep  /dev/root | awk '{print $5}' | head -c -2 | awk '{print 100 -$1}' ").c_str()));
